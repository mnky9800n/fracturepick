...
#zbigs = [1271,1280,1290,1299,1308,1317,1327,1336,1345,1354,1363,1373,1382,1391,1400,1409,1419,1428,1437,1446,1455,1465,1474,1483,1492,1502,1511,1520,1529,1538,1548,1557,1566,1575,1584,1594,1603,1612,1621,1631,1640,1649,1658,1667,1677,1686,1695,1704,1713,1723,1732,1741,1750,1759,1769,1778,1787,1796,1806,1815,1824,1833,1842,1852,1861,1870,1879,1888,1898,1907,1916,1925,1935,1944,1953,1962,1971,1981,1990,1999]
...
txtf = 'output/best_segment_z5_'+noistr+'_'+sizestr+'_hor2D.txt'
totlines = "z noise method accur_frac accur_sol mean_acc poro std mean\n"
for imn in zbigs:
    imstr = str(imn)
    while len(imstr)<4:
        imstr = '0'+imstr
    ...
    isfractg = fractrue==True
    isfrac = isfractg.flatten()
    fracvol = np.sum(isfrac)
    issoltg = fractrue==False
    issol = issoltg.flatten()
    solvol = np.sum(issol)
    
    fracnum_obs = sum(isfractg.flatten())
    solnum_obs = sum(issoltg.flatten())
    
    poro = fracvol/(fracvol+solvol)
    
    fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(1,5)
    ax1.imshow(imtrue, cmap='Greys_r')
    ax1.axis('off')
    ax1.invert_yaxis()
    ax1.set_title('z='+imstr+' '+str(round(poro*100, 1))+'%', fontsize=8)
...
#        cmax = nmu+nsig
        ...
        ax2.imshow(image, cmap = 'Greys_r',  vmin=cmin, vmax =cmax)
        ax2.axis('off')
        ax2.invert_yaxis()
        ax2.set_title('n='+str(ni), fontsize=8)
        ...
        while mi<=3:
            ...
            ax.imshow(frac, cmap = 'Greys_r')
            ax.axis('off')
            ax.invert_yaxis()
            ax.set_title(str(round(accur, 2))+', '+str(round(accurfrac, 2))+", "+str(round(poro*100, 1))+'%', fontsize=7)

            
            frmt= "%s %d %d %f %f %f %f %f %f\n" % (imstr, ni, mi, accurfrac, accursol, accur, poro, nsig, nmu)
...
#            plt.show()
        ...
        #plt.clim(tmu-tsig, tmu+tsig)
        plt.savefig("figs/best/"+sizestr+"_"+noistr+"_"+imstr+"_n"+nstr+".eps", dpi=200, transparent=True)
        #plt.show()
